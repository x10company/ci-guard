...
# см. свод hod-raboty, шаг 2. Труба `curl | bash` убрана 21.09.2026: она
# исполняет то, что лежит по адресу в эту секунду, а набор статический.
#
# Этот скрипт остаётся для ОБНОВЛЕНИЯ набора в уже живом репозитории: он
# различает файлы, которые не перезаписываются никогда, и те, что обновляются
# с резервной копией. Запускают его с машины, где он лежит локально:
#
#   bash /путь/к/ci-guard/primer/postavit.sh
#
