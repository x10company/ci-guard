...
PATTERNS = [
    ...
    # Логин с паролем внутри адреса: socks5://login:pass@host:port.
    #
    # Зачем отдельным правилом. 18.09.2026 страж назвал «чисто» двенадцать
    # файлов, где пароль боевой прокси стоял в сорока одной строке ровно в
    # таком виде. Мимо прошли все правила разом, и каждое по своей причине:
    # имени слева нет (значение лежит в элементе списка), на известные образцы
    # пароль провайдера не похож, а энтропии ему не хватает — такие пароли
    # короткие и произносимые. То же сказала и свежая копия из ci-guard.
    #
    # Ловим оба порядка, потому что в ходу оба: стандартный
    # scheme://логин:пароль@хост:порт и наш реестровый
    # scheme://хост:порт@логин:пароль. Различать их правилу не нужно — в обоих
    # случаях в строке лежит пароль.
    #
    # Заглушка в начале гасит сборку адреса из переменных, а она как раз и есть
    # правильное решение: "socks5://%s:%s@%s:%d" и
    # f'socks5://{USER}:{PASS}@{HOST}:{PORT}' — находкой быть не должны.
    # Обратная сторона: пароль с процентом (URL-кодированный %40) правило
    # пропустит. Это осознанный обмен — ложные срабатывания на каждом
    # правильно написанном файле дороже.
    ("логин с паролем в адресе", re.compile(r"""(?ix)
        \b [a-z][a-z0-9+.-]{1,15} ://
        (?! [^\s@'"]* [${}%<>*] )
        [A-Za-z0-9_.~+-]{2,64} :
        [A-Za-z0-9_.~+-]{4,64} @
...
ALLOW = [
    ...
    # Образец формы адреса в пояснении: «значение вида type://host:port@login:pass».
    # На месте логина и пароля стоят слова-заглушки, а не значения. Нужно для
    # правила «логин с паролем в адресе»: без этого страж ругается на
    # собственные пояснения, а ругающееся зря правило выключают первым.
    re.compile(r"(?i)\b(?:host:port|login:pass(?:word)?|user(?:name)?:pass(?:word)?)\b"),
]
...
PROBY = [
    ...
    # Пароль прокси в адресе, элементом списка — ровно так он и лежал в
    # двенадцати файлах, мимо всех прочих правил. Пароль нарочно короткий и
    # произносимый: на длинном и случайном сработала бы энтропия, и проба
    # проверяла бы не то правило, что нужно.
...
def main():
    ...
    #
    # Кроме одного случая: при проверке ПОДГОТОВЛЕННОГО к коммиту ноль означает
    # «коммитить нечего», и об этом скажет сам git. 07.09.2026 правило дало тут
    # ложное срабатывание: правка не применилась, индекс остался пуст, и хук
    # отказал сообщением про недоступный путь. Ложная тревога опаснее молчания —
    # она учит не доверять и настоящей.
    if skolko == 0 and mode == "staged":
        print("  подготовленных файлов нет — коммитить нечего, это не ошибка")
        return 0
